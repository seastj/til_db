...
- 관계형 DB 관리 : `SQL 문법` 사용
- MySql(Maria DB 와 매우 흡사), Oracle, PostgreSQL(supabase) 등
...
### 5.2. Query Editor 가 PowerShell 이라고 생각한다.
...
### 6.8. `authors 테이블의 id가 1인 데이터`를 posts 테이블에 추가
...
- 여러 칼럼을 정렬함. (앞쪽 처리 후, 나머지 순으로 처리됨)
